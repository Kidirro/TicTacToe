...
namespace Cards
{
    ...
    public class CardInfo : ScriptableObject
    {
        ...
        {
            Vector4 CurrentArea = AreaController.GetArea(CardPoolController.chosedCell, CardAreaSize);
        ...
        public void PlaceRandom5()
        {
            for (int i = 0; i < 5; i++)
            {
                Field.Instance.PlaceInRandomCell();
                FinishLineManager.Instance.MasterChecker(PlayerManager.Instance.GetCurrentPlayer().SideId);
            }
        }
        ...
        {
            Field.Instance.FreezeCell(CardPoolController.chosedCell);


            EffectManager.Instance.FreezeCell_Effect(CardPoolController.chosedCell);
            NetworkEventManager.RaiseEventAddFreezeEffect(CardPoolController.chosedCell);
        ...
        public void FreezeCellGroup()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                while (_posList.IndexOf(Field.Instance.CellList[result.x][result.y]) != -1)
                    result = AIManager.Instance.GenerateRandomPosition(Field.Instance.FieldSize);
        ...
        {
            List<Cell> AllCells =
                Field.Instance.GetAllCellWithFigure((CellFigure) PlayerManager.Instance.GetCurrentPlayer().SideId);
        ...
        public void BreakFreeze()
        {
            ...
            CoroutineQueueController.Instance.AddAwaitTime(resultEffects[0].EffectTimeDisable);
        ...
        public void PlaceAroundFreeze()
        {
            ...
            for (int i = 0; i < Field.Instance.CellList.Count; i++)
                _allFreeze.AddRange(Field.Instance.CellList[i].FindAll(x => x.SubState == CellSubState.freeze));
        ...
        public void FreezeAllMana()
        {
            ...
            ManaManager.Instance.UpdateManaUI();
            CoroutineQueueController.Instance.AddAwaitTime(Cell.AnimationTime);
            FinishLineManager.Instance.MasterChecker(PlayerManager.Instance.GetCurrentPlayer().SideId);
